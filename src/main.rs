//! `api_to_sql` is a small CLI that:
//! 1) fetches JSON from an API,
//! 2) unifies object arrays into a single representative object, and
//! 3) generates SQL DDL for a table that can store the unified shape.
...
struct Cli {
    /// The selected CLI subcommand.
    #[command(subcommand)]
    command: Commands,
}

/// Supported CLI subcommands for each stage of the pipeline.
...
/// Fetches JSON from a URL and writes it to disk.
///
/// # Arguments
/// * `url` - Fully-qualified API URL that returns JSON.
/// * `out` - Destination file path for serialized JSON.
...
/// Reads an input JSON file, extracts an array at `path`, unifies all objects,
/// and writes the unified object to disk.
///
/// # Arguments
/// * `input` - Source JSON file path.
/// * `path` - Dot path (`a.b.c`) or JSON pointer (`/a/b/c`) to an array.
/// * `out` - Destination file path for the unified JSON object.
...
/// Reads a unified JSON object and generates `CREATE TABLE` SQL.
///
/// # Arguments
/// * `input` - Source path to the unified JSON file.
/// * `table` - Requested SQL table name (sanitized before output).
/// * `out` - Optional destination `.sql` file; if omitted, prints to stdout.
...
/// Resolves `path` to an array reference within `root`.
///
/// Supports two path syntaxes:
/// * Dot path (`properties.periods`)
...
/// Builds a unified object from an array of JSON objects.
///
/// The union keeps the first non-object value seen for a key and recursively
/// merges nested object keys.
...
/// Recursively merges keys from `src` into `dst`.
///
/// If both values for the same key are objects, the merge continues recursively.
/// Otherwise, the value already in `dst` is retained.
...
/// Flattens a nested JSON object into SQL-like `(column_name, sql_type)` pairs.
///
/// Nested object keys are concatenated with `_` to produce a flat column name.
...
/// Converts arbitrary text into a conservative SQL identifier.
///
/// Rules:
/// * Lowercase letters and keep alphanumeric/underscore.
...
/// Infers a Postgres-friendly SQL type from a JSON value.
...
/// Builds a `CREATE TABLE` statement from a table name and flattened columns.
